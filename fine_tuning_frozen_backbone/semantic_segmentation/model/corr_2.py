...
def calculate_p_values(data_raw, metric):
    ...
    for i, seed in enumerate(sorted(data["seed"].unique())):
        ...
        mean_fl = data_raw[(data["seed"] == seed) & (data["backbone"] == "FL-Backbone") & (data_raw["class_observed"] == 1)][metric].mean()
        mean_cen = data_raw[(data["seed"] == seed) & (data["backbone"] == "CEN-Backbone") & (data_raw["class_observed"] == 1)][metric].mean()
        ...
        # if p < 0.01:
        #     print(f"Statistical relevant: p-value={p} < 0.01 --> Reject H0 (the distributions of the two samples are equal)")
        # else:
        #     print(f"Statistical not relevant: p-value={p} >= 0.01 --> Fail to reject H0: (the distributions of the two samples are not equal)")
        p_values.append(p)
...
def create_half_violin_plots(csv, metric, resolution, safe=False):
    ...
    fig.suptitle(f'Surgical Scene Segmentation ({resolution} | Frozen Backbone)', fontsize=30, weight='bold', y=0.98)  # Add overall title
    ...
    if safe:
        ...
        plt.savefig(f"{base}/sss_frozen/{metric}_{resolution}.png")
    plt.show()
...
def create_half_violin_plots_small(csv, metric, resolution, safe=False):
    ...
    fig.suptitle(f'Surgical Scene Segmentation ({resolution} | Fully Fine-Tuned)', fontsize=25, weight='bold',
                 y=1.0)  # Add overall title
...
if __name__ == '__main__':
    create_half_violin_plots("sss_frozen.csv", "iou", "low_res", safe=True)
    create_half_violin_plots("sss_frozen.csv", "iou", "high_res", safe=True)

    # create_half_violin_plots_small("sss_frozen.csv", "iou", "low_res")
    # print('********************************************************************************')
    # create_half_violin_plots_small("sss_frozen.csv", "iou", "high_res")
