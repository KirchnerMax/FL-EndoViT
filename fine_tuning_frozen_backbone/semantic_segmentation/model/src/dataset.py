...
from src.custom_transforms import custom_RandomResizedCrop, custom_Resize, custom_RandomHorizontalFlip, \
    custom_Normalize, custom_ToTensor
...
class CholecSeg8k(Dataset):
    def __init__(self, data_dir, RP_file, dataset_type="train", transform=None, vids_to_take=None, verbose=True,
        ...
        self.test_endovit = test_endovit

        assert dataset_type in ["train", "val",
                                "test"], f"When building CholecSeg8k dataset, argument \"dataset_type\" should be one of the following [train, val, test], received \"dataset_type={dataset_type}\" instead."
        ...
        assert self.relative_paths.shape == (8080,
                                             5), f"Expected \"Relative Paths\" to be of shape (8080, 5), but received the shape {self.relative_paths.shape}"
        ...
        if (vids_to_take is None):
            ...
            self.relative_paths = self.relative_paths.loc[
                self.relative_paths["image"].apply(lambda x: int(x.split("/")[0][-2:]) in self.vids_to_take)]
    ...
    def __getitem__(self, index):
        ...
        if (self.transform is not None):
            ...
            if self.test_endovit:
                sample['img_path'] = str(img_path)
                sample['ground_truth_path'] = str(ground_truth_path)
