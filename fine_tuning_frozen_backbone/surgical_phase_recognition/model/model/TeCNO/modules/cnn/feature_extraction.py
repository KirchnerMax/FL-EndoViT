...
class FeatureExtraction(LightningModule):
    def __init__(self, hparams, model, dataset):
        ...
        self.num_tasks = self.hparams.num_tasks  # outputs stem 0, outputs phase 1 , outputs phase and tool 2
        # CHANGED: Automatic balancing can optionally be used.
    ...
    def loss_phase_tool(self, p_phase, p_tool, labels_phase, labels_tool, num_tasks):
        ...
        # CHANGED: automatic balancing
        # because: Automatic balancing is supposed to give equal weight to both losses, even when one of them is
        #          smaller. For our project we believe giving loss_tool more attention is unnecessary, but we will
        #          keep it on, as TeCNO uses it. We didn't notice a difference in runs when it was on and
        #          when it was off.
        if self.hparams.loss_balancing:
    ...
    # ADDED: we rename the function "save_to_drive" to "calculate_phase_accuracy"
    def calculate_phase_accuracy(self, vid_index):
        acc, ppv, tpr, keys, f1 = self.get_phase_acc(self.current_phase_labels,
                                                     self.current_p_phases)

        # ADDED: moved here from below
        self.test_acc_per_video[vid_index] = acc

        # ADDED: We should extract features only if the hparams.test_extract is True.
        if self.hparams.test_extract:
            save_path = self.pickle_path / f"{self.hparams.fps_sampling_test}fps"
            save_path.mkdir(exist_ok=True)
            save_path_txt = save_path / f"video_{vid_index:02d}_{self.hparams.fps_sampling_test}fps_acc.txt"
            save_path_vid = save_path / f"video_{vid_index:02d}_{self.hparams.fps_sampling_test}fps.pkl"
    ...
    def test_step(self, batch, batch_idx):
        ...
        # because:
        # If hparams.test_extract == True,
        # the line above will measure accuracy on everything (train, val and test dataset).
        # Even if hparams.test_extract == False,
        # it measures the accuracy of the test dataset. Our reported accuracy should instead
        # be the mean of accuracies per video.
        # For that reason we remove it.
        # END_REMOVED
        ...
        for i in range(len(vid_idxs)):
            ...
            if vid_idx != self.current_video_idx:
                # "save_to_drive" was renamed to "calculate_phase_accuracy":
                self.calculate_phase_accuracy(self.current_video_idx)
        ...
        if (batch_idx + 1) * self.hparams.batch_size >= self.len_test_data:
            # "save_to_drive" was renamed to "calculate_phase_accuracy":
            self.calculate_phase_accuracy(vid_idx)
            print(f"Finished extracting all videos...")



    def test_epoch_end(self, outputs):
        # ADDED: We now only report the mean of the accuracies on individual videos from the test dataset. I.e.
        #        the original "test_acc_test".
        self.log("test_acc", np.mean(np.asarray([self.test_acc_per_video[x] for x in
                                                 self.dataset.vids_for_test])))
        # REMOVED: other accuracies
        # If hparams.test_extract == False Val and Train accuracy will break the code
        # Moreover, previous "test_acc" is no longer measured.
    ...
    def add_module_specific_args(parser):  # pragma: no cover
        ...
        cholec_fe_module.add_argument("--loss_balancing", action="store_true")
        return parser
