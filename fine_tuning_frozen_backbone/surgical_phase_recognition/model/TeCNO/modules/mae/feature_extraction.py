...
class FeatureExtraction(LightningModule):
    def __init__(self, hparams, model, dataset):
        ...
        # CHANGED: Automatic balancing can opotionally be used.
    ...
    def loss_phase_tool(self, p_phase, p_tool, labels_phase, labels_tool, num_tasks):
        ...
        # CHANGED: automatic balancing is optional now, but we still use it in our tests.
    ...
    def configure_optimizers(self):
        ...
        """
        # ADDED
        if (self.model.param_groups is not None):
                for group in self.model.param_groups:
                    group["lr"] = self.hparams.learning_rate * group["lr_scale"]

                optimizer = optim.Adam(self.model.param_groups, lr=self.hparams.learning_rate)
        else:
        # END ADDED
