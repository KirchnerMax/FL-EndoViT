...
def train(kfold=0):
    ...
    ds_train = GynSurgAction(
        root="/mnt/cluster/datasets/GynSurg_Action_3sec/GynSurg_smoke_dataset",
        df_path='./df_smoke.csv',
        split='train',
        k_fold=kfold,
    ...
    model = MAEViTLSTMClassifier(lstm_hidden_size=256, lstm_layers=2, mae_kwargs={
        ...
        'mae_ckpt': '/mnt/ceph/tco/TCO-Staff/Homes/kirchnema/04_pycharm/05_endovit_fl/00_original_code/EndoViT/pretraining/pretrained_endovit_models/EndoViT_for_ActionTripletDetection/endovit_ATD.pth',  # path to checkpoint if available
    ...
    for epoch in range(epochs):
        ...
        if epoch_f1 is not None:
            if epoch_f1 > best_f1:
                ...
                try:
                    ckpt_name = f"best_CEN_ENDOVIT_fold_{kfold}.pth"
    ...
    out_csv = f"test_CEN_ENDOVIT_predictions_fold_{kfold}.csv"
    try:
        import csv
    ...
    plt.savefig(f'confusion_matrix_CEN_ENDOVIT_fold_{kfold}.png')
    plt.close()
    print(f"Saved confusion matrix to confusion_matrix_CEN_ENDOVIT_fold_{kfold}.png")
