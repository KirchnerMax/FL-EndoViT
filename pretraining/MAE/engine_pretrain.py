...
def show_image(args, axis, image, title='', alpha=1):
    ...
    means = args.dataloader.mean.strip('[]').split(',')
    float_means = list(map(float, means))
    means = torch.FloatTensor(float_means)

    stds = args.dataloader.std.strip('[]').split(',')
    float_stds = list(map(float, stds))
    stds = torch.FloatTensor(float_stds)

    # Reshape to be broadcastable to the shape of the image
    dataset_mean = means.view(1, 1, 3)
    dataset_std = stds.view(1, 1, 3)

    axis.imshow(torch.clip((image * dataset_std + dataset_mean) * 255, 0, 255).int())
...
def train_and_validate_one_epoch(
    ...
    if fedprox:
        global_param = copy.deepcopy(model).parameters()
    else:
        global_param = None
    model.train(True)
    ...
    header = 'Client [{}] - Epoch: [{}]'.format(c_name, epoch)
    print_freq = 20
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader_train, print_freq, header)):
        ...
        # use swa scheduler
        if args.do_swa:
            if (data_iter_step + 1) % args.swa_average_every_n_steps == 0 or (data_iter_step + 1) == len(
                    data_loader_train):  # ADDED 10.02.2023. update the parameters before evaluation only (instead of at every step)
        ...
        with torch.amp.autocast('cuda'):
            proximal_term = torch.tensor(0.0).to(device)
            if fedprox:
                # FedProx loss
                for local_weights, global_weights in zip(model.parameters(), global_param):
                    proximal_term += (local_weights - global_weights.detach()).norm(2)
            losses, _, _, _ = model(samples, mask_ratio=args.training.mask_ratio)
            loss, loss_per_patch = losses
            if fedprox:
                proximal_term = proximal_term * args.fed_prox.mu / 2.0
                loss += proximal_term
        ...
        log_writer.add_scalar(f'train/{cid}_train_loss', loss_value_reduce, epoch_1000x)
        log_writer.add_scalar(f'train/{cid}_lr', max_lr, epoch_1000x)
        ...
        csv_logger.log_metrics(log_msg, epoch, c_name, cid)

        # wandb logging
        if (args.use_wandb and (data_iter_step + 1) % (accum_iter * args.log_every_n_steps) == 0):
            epoch_1000x = int((data_iter_step / len(data_loader_train) + epoch) * 1000)
            set_wandb.setup_wandb(tr_cfg, cid)
            wandb.log(
                log_msg,
        ...
        if ((data_iter_step + 1) == len(data_loader_train)):
            epoch_end = True
            val_stats = evaluate(data_loader_val, model, device, log_writer, epoch, args, c_name=c_name,
                                 epoch_end=epoch_end, is_swa=False, cid=cid, tr_cfg=tr_cfg)

            # evalute SWA model separately
            if (args.do_swa and swa_model is not None):
                val_stats_swa = evaluate(data_loader_val, swa_model, device, log_writer, epoch, args, c_name=c_name,
                                         epoch_end=epoch_end, is_swa=True, cid=cid, tr_cfg=tr_cfg)
        ...
        if (val_stats):
            ...
            log_stats(log_writer, val_stats, epoch_1000x, args, csv_logger, epoch_end=epoch_end, is_swa=False, cid=cid,
                      tr_cfg=tr_cfg)
            if (args.do_swa and val_stats_swa):
                log_stats(log_writer, val_stats_swa, epoch_1000x, args, csv_logger, epoch_end=epoch_end, is_swa=True, cid=cid,
                          tr_cfg=tr_cfg)
...
def evaluate(data_loader,
    ...
    if (epoch_end):
        header = f'###### Client: [{c_name}] Epoch: [{epoch}] {swa_text}Validation ######'
    else:
        header = f'{swa_text}Validation Client [{c_name}] at epoch [{epoch}] step: [{kwargs["step"]}/{kwargs["total_steps"]}]'
    ...
    if (not epoch_end):
        for i, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
            ...
            with torch.inference_mode():
                ...
                losses, y, mask, latent = model(images, mask_ratio=args.training.mask_ratio)
    ...
    # plot embeddings with tensorboard at the end of the epoch
        ...
        dataset_mean = means.view(1, 3, 1, 1)
        dataset_std = stds.view(1, 3, 1, 1)
        ...
        for i, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
            ...
            patches_below_0_3 = torch.where(loss_per_patch < 0.3, 1, 0).sum() / args.dataloader.batch_size
            patches_below_0_1 = torch.where(loss_per_patch < 0.1, 1, 0).sum() / args.dataloader.batch_size
            patches_below_0_0_5 = torch.where(loss_per_patch < 0.05, 1, 0).sum() / args.dataloader.batch_size
            patches_below_0_0_1 = torch.where(loss_per_patch < 0.01, 1, 0).sum() / args.dataloader.batch_size
            ...
            if i in print_steps:
                ...
                figure_to_add = plot_results(args, [images[index_to_take], im_masked[index_to_take], y[index_to_take],
                                                    im_paste[index_to_take]], loss_per_patch[index_to_take])

                log_writer.add_figure(f'val/{cid}_Example_{str(i)}_{swa_text}Results',
                                      figure_to_add,
                                      global_step=epoch)
                ...
                if (args.use_wandb):
                    ...
                    wandb.log(
                        ...
                        commit=False
                        # Never commit here because if we are at the epoch end, we have one more wandb.log call.
                    )
                    wandb.finish()
                else:
                    print("Not using wandb - see tensorboard for image")
        ...
        log_writer.add_embedding(torch.cat(embeddings, dim=0),
                                 ...
                                 global_step=epoch,
                                 tag=f"{cid}_{swa_text}Embedding_Visualization")

        # gather the stats from all processes
        metric_logger.synchronize_between_processes()
        print('* {swa_text}Validation loss {losses.global_avg:.4f}'.format(
            losses=metric_logger.loss if not is_swa else metric_logger.loss_swa, swa_text=swa_text))

    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}


# Pushed model saving into a separate function to allow saving both swa and normal model separately.
def save_model_based_on_validation_results(model, optimizer, loss_scaler, val_stats, best_result_dict, args, epoch,
                                           epoch_end=False, is_swa=False):
    if args.log_dir:
        ...
        if (best_result_dict["loss"] > val_stats['loss' if not is_swa else 'loss_swa'] or (
                epoch_end and (epoch % 2 == 0 or epoch + 1 == args.training.num_rounds))):
            if (args.distributed):
                model_to_save = model
            ...
            if (best_result_dict["loss"] > val_stats['loss' if not is_swa else 'loss_swa']):
                ...
                print(
                    f"Best checkpoint saved! Epoch: {epoch} Val Loss: {val_stats['loss' if not is_swa else 'loss_swa']:.4f}" if not is_swa else
            ...
            if (epoch_end and (epoch % 2 == 0 or epoch + 1 == args.training.num_rounds)):
...
def log_stats(log_writer, val_stats, epoch_1000x, args, csv_logger, epoch_end=False, is_swa=False, cid=None, tr_cfg=None):
    # log the test stats in both cases
    suffix = "" if not is_swa else "_swa"

    if log_writer is not None:
        log_writer.add_scalar(f'val/{cid}_val_loss{suffix}', val_stats['loss' if not is_swa else 'loss_swa'], epoch_1000x)

        if (epoch_end):
            log_writer.add_scalar(f'val/{cid}_patches_below_0_3{suffix}',
                                  val_stats['patches_below_0_3' if not is_swa else 'patches_below_0_3_swa'],
                                  epoch_1000x)
            log_writer.add_scalar(f'val/{cid}_patches_below_0_1{suffix}',
                                  val_stats['patches_below_0_1' if not is_swa else 'patches_below_0_1_swa'],
                                  epoch_1000x)
            log_writer.add_scalar(f'val/{cid}_patches_below_0_0_5{suffix}',
                                  val_stats['patches_below_0_0_5' if not is_swa else 'patches_below_0_0_5_swa'],
                                  epoch_1000x)
            log_writer.add_scalar(f'val/{cid}_patches_below_0_0_1{suffix}',
                                  val_stats['patches_below_0_0_1' if not is_swa else 'patches_below_0_0_1_swa'],
                                  epoch_1000x)

    # wandb logging and csv logging
    ...
    csv_logger.log_metrics(log_dict, epoch_1000x/1000, DSName(cid), cid)
    ...
    if (epoch_end):
        log_msg = {
                f"val/patches_below_0_3{suffix}": val_stats[
                    'patches_below_0_3' if not is_swa else 'patches_below_0_3_swa'],
                f"val/patches_below_0_1{suffix}": val_stats[
                    'patches_below_0_1' if not is_swa else 'patches_below_0_1_swa'],
                f"val/patches_below_0_0_5{suffix}": val_stats[
                    'patches_below_0_0_5' if not is_swa else 'patches_below_0_0_5_swa'],
                f"val/patches_below_0_0_1{suffix}": val_stats[
                    'patches_below_0_0_1' if not is_swa else 'patches_below_0_0_1_swa'],
            }

        csv_logger.log_metrics(log_msg, epoch_1000x/1000, DSName(cid), cid)
