...
Inputs:
    ...
    -> nb_classes (int): The number of classes the final linear layer should outputs. Set this number to 0 to remove the final
                      linear layer (i.e. the "head" layer).
    
    -> drop_path (float): drop_path value of the ViT model. Default 0.1.
    
    -> pool_type ("str"): The outputs of final attention block of ViT encoder will have the following shape: [B, N, E],
                       where B=batch_size, N=number_of_patches, E=embedding_dimension. This outputs first gets pooled and then
                       forwarded to the final linear layer ("head"). 
                       If pool_type=None no pooling is applied. That is we outputs per-patch embeddings.
    ...
    -> return_optimizer_groups (bool): Whether or not to prepare a list of MAE model parameter groups which you can then pass to the optimizer
                                    of your training procedure. The outputs will be a list of dictionaries with three keys:
...
def prepare_mae_model(model_type, nb_classes, drop_path, pool_type, mae_ckpt, freeze_weights, reinit_n_layers=-1, return_optimizer_groups=False, weight_decay=0., layer_decay=1.0, verbose=False, debug=False):
    # build model
    ...
    if (return_optimizer_groups):
        param_groups = lrd.param_groups_lrd(model, weight_decay,
                                            ...
                                            layer_decay=layer_decay, verbose=False)
