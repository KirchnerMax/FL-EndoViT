...
import MAE.util.lr_sched_optuna as lr_sched
...
def train_and_validate_one_epoch(
    ...
    minimizer = ASAM(optimizer, model, rho, eta)
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader_train, print_freq, header)):
        ...
        if args.do_swa:
            if (data_iter_step + 1) % args.swa_average_every_n_steps == 0 or (data_iter_step + 1) == len(
                ...
                lr_sched.adjust_learning_rate(optimizer, data_iter_step / len(data_loader_train) + epoch, args, lr)
        ...
        if (val_stats):
            ...
            log_stats(log_writer, val_stats, epoch_1000x, args, epoch_end=epoch_end, is_swa=False, cid=cid,
                      tr_cfg=tr_cfg)
            if (args.do_swa and val_stats_swa):
                log_stats(log_writer, val_stats_swa, epoch_1000x, args, epoch_end=epoch_end, is_swa=True, cid=cid,
                          tr_cfg=tr_cfg)
...
def log_stats(log_writer, val_stats, epoch_1000x, args, epoch_end=False, is_swa=False, cid=None, tr_cfg=None):
