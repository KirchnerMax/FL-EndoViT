...
def main(csv, metric='iou', n_videos='total'):
    ...
    for resolution in metrics.keys():
        ...
        print(f"FL-B std mAP: {np.std(metrics[resolution]['FL-Backbone'])}")
        print(f"CEN-B std mAP: {np.std(metrics[resolution]['CEN-Backbone'])}")
...
if __name__ == "__main__":
    main(csv='sss.csv', metric='iou')

    main(csv='sss.csv', metric='iou', n_videos=1)
    main(csv='sss.csv', metric='iou', n_videos=2)
    main(csv='sss.csv', metric='iou', n_videos=4)
