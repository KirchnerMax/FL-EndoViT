...
class TeCNO(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        print("recall", recall)
        # calculate average precision, recall, ignore nan values
        a_pre = torch.mean(precision[~precision.isnan()])
        a_rec = torch.mean(recall[~recall.isnan()])
        f1_score_per_video = 2 * (a_pre * a_rec) / (a_pre + a_rec)
        print('##############################################')
        print("f1_score_per_video: ", f1_score_per_video)
        print('##############################################')
        self.val_acc_stages(y_pred, y_true)
        acc_stages = self.val_acc_stages.compute()
        for s in range(len(acc_stages)):
            print(f"Stage S{s + 1} Accuracy: {acc_stages[s]}")
