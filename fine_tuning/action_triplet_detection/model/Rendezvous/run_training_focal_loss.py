...
parser.add_argument('--hr_output', action='store_true', help='Whether to use higher resolution outputs (32x56) or not (8x14). Default: False')
# In this code not used:
...
# output_dir
parser.add_argument('--output_dir', type=str, default="./__checkpoint__",  help='Path to outputs directory.')
...
parser.add_argument('--nb_classes', default=100, type=int,
                    help='number of the features in the outputs of the last layer (head)') # set it to 0 to remove the last layer
    ...
    with open(stdout_file_path, 'w') as f_out, redirect_stdout(f_out):
        ...
        def train_loop(dataloader, model, activation, loss_fn_ivt, optimizer, scheduler, epoch, n_steps_per_epoch, logfile, per_class_weights, **kwargs):
            ...
            for batch, (imgs, (y1, y2, y3, y4)) in enumerate(dataloader):
                imgs, y4 = imgs.cuda(), y4.cuda()        # we only outputs predictions for triplets
                # print(f'imgs.shape: {imgs.shape}, y4.shape: {y4.shape}')
                model.train()
                triplet = model(imgs)
                logit_ivt       = triplet                
        ...
        def test_loop(dataloader, model, activation, loss_fn_ivt, mAP_tracker, acc_tracker, per_class_weights, is_test = False, last_video=False, print_qualitative_examples=False, epoch=0):
            ...
            # calculate mAP for current video
            mAP_ivt_video = mAP_tracker.compute_AP('ivt')
            print(f"mAP_ivt_video {i}: {mAP_ivt_video['mAP']}", file=open(logfile, 'a+'))
            print(f"mAP_ivt_video {i} class wise: {mAP_ivt_video['AP']}", file=open(logfile, 'a+'))
        ...
        if is_test:
            ...
            print(f"len(test_dataloaders): {len(test_dataloaders)}")
